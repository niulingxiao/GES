...
# from res2net import res2net50_v1b_26w_4s, res2net50_v1b_26w_4s_modified
...
class GeneralizedMeanPooling(nn.Module):
    ...
    def forward(self, x):
        # print(x)
        x = torch.pow((x.clamp(min=self.eps)), self.p).clamp(min=1e-32)
        # x = ((x.clamp(min=self.eps)).pow(self.p).clamp(min=1e-32))
        # print(x)
        # print(x)
        return torch.pow(torch.nn.functional.adaptive_avg_pool2d(x, self.output_size), (1.0 / self.p))
...
class MVSS_res2net(nn.Module):
    def __init__(self, nclass, aux=False, sobel=False, constrain=False, n_input=3, input_image_size=512, **kwargs):
        super(MVSS_res2net, self).__init__()
        self.num_class = nclass
        self.aux = aux
        self.input_image_size = input_image_size

        # Encoder edge
        resnet1 = res2net50_v1b_26w_4s_modified(pretrained=True)
        self.encoder1_conv = resnet1.conv1
        self.encoder1_bn = resnet1.bn1
        self.encoder1_relu = resnet1.relu
        self.maxpool = resnet1.maxpool
        self.encoder2 = resnet1.layer1
        self.encoder3 = resnet1.layer2
        self.encoder4 = resnet1.layer3
        self.encoder5 = resnet1.layer4
        # Encoder noise
        resnet2 = res2net50_v1b_26w_4s_modified(pretrained=True)
        self.noise_encoder1_conv = resnet2.conv1
        self.noise_encoder1_bn = resnet2.bn1
        self.noise_encoder1_relu = resnet2.relu
        self.noise_maxpool = resnet2.maxpool
        self.noise_encoder2 = resnet2.layer1
        self.noise_encoder3 = resnet2.layer2
        self.noise_encoder4 = resnet2.layer3
        self.noise_encoder5 = resnet2.layer4
        ...
        self.gmp = nn.MaxPool2d(self.input_image_size)
    ...
    def forward(self, x):
        ...
        input_ = x.clone()
        # Encoder edge
        e1 = self.encoder1_conv(input_)
        e1 = self.encoder1_bn(e1)
        e1 = self.encoder1_relu(e1)
        e1_pool = self.maxpool(e1)
        e2 = self.encoder2(e1_pool)  # 256,128,128
        e3 = self.encoder3(e2)  # 512,64,64
        e4 = self.encoder4(e3)  # 1024,32,32
        e5 = self.encoder5(e4)  # 2048,16,16

        if self.sobel:
            res1 = self.erb_db_1(run_sobel(self.sobel_x1, self.sobel_y1, e2))
            res1 = self.erb_trans_1(res1 + self.upsample(self.erb_db_2(run_sobel(self.sobel_x2, self.sobel_y2, e3))))
            res1 = self.erb_trans_2(res1 + self.upsample_4(self.erb_db_3(run_sobel(self.sobel_x3, self.sobel_y3, e4))))
            res1 = self.erb_trans_3(res1 + self.upsample_4(self.erb_db_4(run_sobel(self.sobel_x4, self.sobel_y4, e5))), relu=False)

        else:
            res1 = self.erb_db_1(e2)
            res1 = self.erb_trans_1(res1 + self.upsample(self.erb_db_2(e3)))
            res1 = self.erb_trans_2(res1 + self.upsample_4(self.erb_db_3(e4)))
            res1 = self.erb_trans_3(res1 + self.upsample_4(self.erb_db_4(e5)), relu=False)

        if self.constrain:
            x = rgb2gray(x)
            x = self.constrain_conv(x)
            # Encoder noise
            n1 = self.noise_encoder1_conv(x)
            n1 = self.noise_encoder1_bn(n1)
            n1 = self.noise_encoder1_relu(n1)
            n1_pool = self.noise_maxpool(n1)
            n2 = self.noise_encoder2(n1_pool)
            n3 = self.noise_encoder3(n2)
            n4 = self.noise_encoder4(n3)
            n5 = self.noise_encoder5(n4)
            e5 = torch.cat([e5, n5], dim=1)
        ...
        out_labels = self.gmp(x0).squeeze()

        return res1, x0, out_labels
...
class ConvGeM(nn.Module):
    def __init__(self, q):
        ...
        self.conv2 = nn.Conv2d(32, 1, kernel_size=1, stride=1, padding=0)
        self.relu = nn.ReLU()
        self.bn = nn.BatchNorm2d(32)
        self.q = q
        # self.conv3 = nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1)

    def forward(self, x, lamda=True):
        x2 = x.clone()
        x = self.conv1(x)
        x = self.relu(x)
        x = self.bn(x)
        x = self.conv1(x)
        x = torch.sigmoid(x)
        x = GeneralizedMeanPoolingP(x)
        # res = self.conv3(res)
        x2 = GeneralizedMeanPoolingP(x2)

        if lamda:
            return self.q * x2 + (1.0 - self.q) * x
        else:
            return x
...
class MVSSNetplus(ResNet50):
    ...
    def forward(self, x):
        ...
        # edge feature, mask, image-level-predict
        return res1, x0, g
...
def get_mvss(backbone='resnet50', pretrained_base=True, nclass=1, sobel=True, n_input=3, constrain=True, input_image_size=512, **kwargs):
...
def get_mvss_res2net(backbone='resnet50', pretrained_base=True, nclass=1, sobel=True, n_input=3, constrain=True, input_image_size=512,  **kwargs):
    model = MVSS_res2net(nclass, backbone=backbone,
                        pretrained_base=pretrained_base,
                        sobel=sobel,
                        n_input=n_input,
                        constrain=constrain,
...
def get_mvssplus(backbone='resnet50', pretrained_base=True, nclass=1, sobel=True, n_input=3, constrain=True, **kwargs):
    model = MVSSNetplus(nclass, backbone=backbone,
                        pretrained_base=pretrained_base,
                        sobel=sobel,
                        n_input=n_input,
                        constrain=constrain,
...
if __name__ == '__main__':
    ...
    model = get_mvss_res2net(sobel=True, n_input=3, constrain=True)

    edge, outputs, out_labels = model(img)
    out_edges = torch.sigmoid(edge)
    out_masks = torch.sigmoid(outputs)
    # print(out_masks)

    gmp = nn.MaxPool2d(512)
    gem = GeneralizedMeanPooling(512)
    out_put = gem(out_masks)
    ...
    model.eval()
    model.to(device)
    dump_input = img.to(device)
    for _ in range(5):
        start = time.time()
        edge, outputs, out_labels = model(dump_input)
    ...
    #     edge, outputs = model(dump_input)
    # print(prof.table())
    # prof.export_chrome_trace('./mvss_profile.json')
